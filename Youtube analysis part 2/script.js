  ...
  try {
    ...
    try {
      ...
      const filteredSubtitles = subtitleOptions.filter(subtitle => 
        ...
        return `Language: ${subtitle}, Code: ${code}`; /// Return the formated subtitle language and code.
      });

      subtitles = filteredSubtitles.join('\n') || 'No subtitles available'; // Join the subtitles into a string or set a defualt value.
    } catch (err) {
